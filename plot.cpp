    ...
    QwtPolarPlot( QwtText( "PPI Display" ), parent )
{
    ...
    setScale(QwtPolar::Azimuth,azimuthInterval.maxValue(),azimuthInterval.minValue(),azimuthInterval.width()/12.0);
    // scales
    ...
    marker2->setPosition( QwtPointPolar( 120,50) );
    marker2->setSymbol( new QwtSymbol( QwtSymbol::DTriangle,
        QBrush( Qt::blue ), QPen( Qt::red ), QSize( 11,11 ) ) );
    marker2->setLabelAlignment( Qt::AlignHCenter | Qt::AlignTop );

    QwtText text2( "JA134" );
    text.setColor( Qt::black );
    QColor bg2( Qt::yellow );
    bg.setAlpha( 200 );
    text2.setBackgroundBrush( QBrush( bg2 ) );

    marker2->setLabel( text2 );
    marker2->attach( this );
